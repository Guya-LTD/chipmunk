 ...
 * @version  1.0.0
 */
class ReceiptEmailController extends AbstractController {
    /**
    ...
    public function sendEmail(Request $request, MailerInterface $mailer){
        try{
            ...
            $email->from($_SERVER['ACCOUNT_SENDER_EMAIL'])
                  ->to($params['to'])
                  ->subject('Guya Shop, Receipt')
                  ->htmlTemplate('emails/receipt-en.html.twig')
                  ->context([
                      'name' => $params['name'],
                      'total' => $params['total'],
                      ...
                      'receipt_details' => $params['receipt_details'],
